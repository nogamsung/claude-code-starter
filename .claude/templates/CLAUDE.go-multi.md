# [프로젝트명] — Go (Go Workspace 멀티 서비스)

## Stack
- **Language**: Go (latest stable)
- **Workspace**: Go Workspace (`go.work`) — 멀티 서비스 구조
- **Framework**: Gin
- **ORM**: GORM (단순 CRUD)
...
| `/new <Resource>` | REST API 전체 스캐폴딩 (멀티 서비스 경로 자동 적용, 명시: `/new api`) |
| `/new module <ServiceName>` | 새 Go 서비스 생성 |
| `/test [파일]` | 테스트 자동 생성 |
...
### Go Workspace 멀티 서비스 구조
```
go.work                              ← use ./services/api ./services/worker ./pkg/shared
services/
  api/                               ← HTTP API 서비스
    go.mod (module github.com/{org}/{project}/services/api)
    cmd/
  ...
  worker/                            ← 백그라운드 워커 서비스 (선택)
    go.mod (module github.com/{org}/{project}/services/worker)
    cmd/main.go
    internal/
pkg/
  shared/                            ← 공유 도메인, 에러, 유틸
    go.mod (module github.com/{org}/{project}/pkg/shared)
    domain/                          ← 서비스 공통 Entity, 공유 타입
    errors/                          ← 공통 에러 정의
```
...
- **`services/api`에서 `services/worker` import 금지** — 단방향 의존만 허용
- **`services/worker`에서 `services/api` import 금지** — 단방향 의존만 허용
- **`pkg/shared`에서 `services/*` import 금지** — 공유 모듈은 순수하게 유지
...
### lint 실행 (각 서비스 디렉토리에서)
```bash
# golangci-lint는 workspace root에서 미지원 — 각 서비스 디렉토리에서 실행
